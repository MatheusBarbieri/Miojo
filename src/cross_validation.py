...
class KFolds:
    ...
    def _random_k_folds(self, add_remaining, seed):
        df = self._data
    ...
    def _stratified_k_folds(self, add_remaining, seed):
        groups = self.data.groupby('class')
    ...
    def _k_folds(self, add_remaining=True, seed=randint(10000)):
        if self._sampling == 'random':
            return self._random_k_folds(add_remaining, seed)
    ...
    def _splits(self, folds):
        for i, fold in enumerate(folds):
            yield pd.concat(folds[:i] + folds[i + 1:]), folds[i]
